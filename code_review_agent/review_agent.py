...
async def review_code(target_dir: str) -> dict:
    prompt = f"""Run `git diff origin/main...HEAD -- ':!.github' ':!code_review_agent'` (or `git diff main...HEAD` if origin/main is unavailable) in {target_dir} to get the changes on this branch.
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Review agent encountered error: {e}", file=sys.stderr)
